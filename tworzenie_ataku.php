...
//echo $id_uczestnik_zalogowany. ' mecz '.$id_meczu.' pozycja '.$wioska_pozycja.' pozycjax '.$pozycja_x.' pozycjay '.$pozycja_y;
echo('<br>');
$sql = "SELECT * FROM wioska WHERE id=$id_wioski";
if($rezultat2 = @$polaczenie->query($sql))
{
    $wioska_lotnisko = $rezultat2->fetch_assoc();
}
$lotnisko=$wioska_lotnisko['lotnisko'];
 $sql = "SELECT * FROM wioska WHERE mecz=$id_meczu AND pozycjax=$pozycja_x AND pozycjay=$pozycja_y";
 ...
 ?>
    ...
    <div class="container">
        ...
        <form id="wojska_ilosc" action="atak_zapis.php" method="POST">
    ...
    <br> <br> 
    <div class=tworzenie onclick="ile_wojska()"> Atakuj</div> 
    </div>
