...
czas=$czas WHERE id=$id_uczestnik_zalogowany";
 ...
 $morale=$wioska_budynki['morale'];
 $sql = "SELECT * FROM morale WHERE id=$morale";
 if($rezultat4 = @$polaczenie->query($sql))
 {
     $wioska_morale = $rezultat4->fetch_assoc();
 }
 $poziom_morale=$wioska_morale['poziom'];
 $morale=$morale+1;
 $sql = "SELECT * FROM morale WHERE id=$morale";
 if($rezultat4 = @$polaczenie->query($sql))
 {
 ...
 if($morale<6){
 $koszt_morale=$wioska_morale['koszt'];
 }
 //echo $poziom_morale;
 $polaczenie->close();
 }
...
<script>
    ...
    <div class="container">
    <br> <br> 
        <div class="nazwa_budynku"> Kościół</div>
        <?php  if($wiersz['era']<=3 ){ ?>
        <center><img class="zdjecie2" src="zdjecia/kosciol.jpg"></center>
        <?php 
        }
        if($wiersz['era']>3 ){ ?>
        <center><img class="zdjecie2" src="zdjecia/kosciol5.jpg"></center>
        <?php 
        }?>
        <br>
        <div class="nazwa_budynku"> Morale poziom <?php echo $wioska_budynki['morale']+1 ?> </div>
        <?php
    if($wioska_budynki['morale']<5){    
    ?>
        <div class="budynek_budowa"> 
  <div class=b_budowa2> Koszt osiągnięcia wyższego morale <?php echo $koszt_morale ?> zł</div>
        <a href="budowa/morale.php">
        <div class="era">PODNIEŚ MORALE</div>
        </a>
    </div>
   <?php
    }
    if($wioska_budynki['morale']==5){
        ?>
        <div class="b_podsumowanie"> Maksymalne dostępne morale
        </div> 
        <?php
    }
   ?>
