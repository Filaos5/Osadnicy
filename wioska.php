...
<script>
    ...
    function surowce_odliczac()
        ...
        //window.onload2 = ataki_odliczac;
    function ataki_odliczac()
        {
            sekundy_liczba = sekundy_liczba-1;
            document.getElementById("zywnosc").innerHTML = sekundy_liczba ;
            setTimeout("ataki_odliczac()",1000);
        }
        function budynek(id){
    zmiennajava = id;
    ...
    <?php 
    $sql = "SELECT * FROM ataki WHERE wioska_koniec=$id_wioski AND czas_dotarcia>$czas ORDER BY czas_dotarcia";
   // $polaczenie->query($sql);
    if($rezultat = @$polaczenie->query($sql))
    {
