<?php 
 ...
 if($rezultat2 = @$polaczenie->query($sql))
 {
     ...
     $wiersz_uniwersytet = $rezultat2->fetch_assoc();
      ...
      $koszt=(int)$wiersz_kosciol['pieniadze']; 
      $drewno_budowa=(int)$wiersz_kosciol['drewno']; 
      $metal_budowa=(int)$wiersz_kosciol['metal']; 
      $kamien_budowa=(int)$wiersz_kosciol['kamien']; 
      if($pieniadze>$koszt && $drewno>$drewno_budowa && $metal>$metal_budowa && $kamien>$kamien_budowa && $pobranywiersz['kosciol']==0){
      $sql = "UPDATE wioska SET kosciol=1 WHERE id=$id_wioski";
