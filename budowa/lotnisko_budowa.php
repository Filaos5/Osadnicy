<?php 
 ...
 if($rezultat2 = @$polaczenie->query($sql))
 {
     ...
     $wiersz_uniwersytet = $rezultat2->fetch_assoc();
      ...
      $koszt=(int)$wiersz_lotnisko['pieniadze']; 
      $drewno_budowa=(int)$wiersz_lotnisko['drewno']; 
      $metal_budowa=(int)$wiersz_lotnisko['metal'];
      $kamien_budowa=(int)$wiersz_lotnisko['kamien'];  
      if($pieniadze>$koszt && $drewno>$drewno_budowa && $metal>$metal_budowa && $kamien>$kamien_budowa && $pobranywiersz['lotnisko']==0){
      $sql = "UPDATE wioska SET lotnisko=1 WHERE id=$id_wioski";
