<?php 
 ...
 if($rezultat2 = @$polaczenie->query($sql))
 {
     ...
     $wiersz_uniwersytet = $rezultat2->fetch_assoc();
      ...
      $koszt=(int)$wiersz_uniwersytet['pieniadze']; 
      $drewno_budowa=(int)$wiersz_uniwersytet['drewno']; 
      $metal_budowa=(int)$wiersz_uniwersytet['metal'];
      $kamien_budowa=(int)$wiersz_uniwersytet['kamien'];  
      if($pieniadze>$koszt && $drewno>$drewno_budowa && $metal>$metal_budowa && $kamien>$kamien_budowa && $pobranywiersz['uniwersytet']==0){
      $sql = "UPDATE wioska SET uniwersytet=1 WHERE id=$id_wioski";
