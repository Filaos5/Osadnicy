<?php 
 ...
 if($rezultat2 = @$polaczenie->query($sql))
 {
     ...
     $wiersz_uniwersytet = $rezultat2->fetch_assoc();
      ...
      $koszt_podstawowy=(int)$wiersz_huta['pieniadze']; 
      $drewno_budowa_podstawowy=(int)$wiersz_huta['drewno']; 
      $metal_budowa_podstawowa=(int)$wiersz_huta['metal'];
      $kamien_budowa_podstawowy=(int)$wiersz_huta['kamien'];  
      $koszt=$koszt_podstawowy*(2**$pobranywiersz['huta']);
      $huta=$pobranywiersz['huta']+1;
      $drewno_budowa=$drewno_budowa_podstawowy*(2**$pobranywiersz['huta']);
      $metal_budowa=$metal_budowa_podstawowa*(2**$pobranywiersz['huta']);
      $kamien_budowa=$kamien_budowa_podstawowy*(2**$pobranywiersz['huta']);
      $czas_budowy=$wiersz_huta['czas_budowy'];
      $koniec_budowy= $czas_budowy+$czas; 
      if($pieniadze>=$koszt && $drewno>=$drewno_budowa && $metal>=$metal_budowa && $kamien>=$kamien_budowa && $pobranywiersz['huta']<5){
        ...
        VALUES ( $huta, 'huta', $id_meczu, $id_wioski, $czas, $koniec_budowy, 0)";
      $polaczenie->query($sql);
        /*
        $sql = "UPDATE wioska SET huta=huta+1, pieniadze=pieniadze-$koszt, drewno=drewno-$drewno_budowa, metal=metal-$metal_budowa, kamien=kamien-$kamien_budowa WHERE id=$id_wioski";
      $polaczenie->query($sql); 
      $sql = "SELECT huta, kuznia FROM wioska WHERE id=$id_wioski";
      if($rezultat = @$polaczenie->query($sql))
      {
          ...
          $sql = "UPDATE uczestnicy SET predkosc_metal=$predkosc_cala WHERE id=$id_wioski";
          $polaczenie->query($sql); 
      }
      */  
       }
