<?php 
 ...
 if($rezultat2 = @$polaczenie->query($sql))
 {
     ...
     $wiersz_uniwersytet = $rezultat2->fetch_assoc();
      ...
      $czas_budowy=$wiersz_blok['czas_budowy'];
      $koniec_budowy= $czas_budowy+$czas;    
      if($pieniadze>$koszt && $kamien>$kamien_budowa && $metal>$metal_budowa){
      $sql = "UPDATE wioska SET pieniadze=pieniadze-$koszt, kamien=kamien-$kamien_budowa, metal=metal-$metal_budowa WHERE id=$id_wioski";
      @$polaczenie->query($sql);
      $sql = "INSERT INTO budowa ( ilosc, budynek, mecz, wioska, kiedy_rozpoczeto, kiedy_koniec, zrobione)
      VALUES ( $bloki_ilosc, 'blok', $id_meczu, $id_wioski, $czas, $koniec_budowy, 0)";
    $polaczenie->query($sql);
       }
       else{
