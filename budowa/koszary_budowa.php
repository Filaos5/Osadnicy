<?php 
 ...
 if($rezultat2 = @$polaczenie->query($sql))
 {
     ...
     $wiersz_uniwersytet = $rezultat2->fetch_assoc();
      ...
      $koszt=(int)$wiersz_koszary['pieniadze']; 
      $drewno_budowa=(int)$wiersz_koszary['drewno']; 
      $metal_budowa=(int)$wiersz_koszary['metal'];
      $kamien_budowa=(int)$wiersz_koszary['kamien'];  
      if($pieniadze>$koszt && $drewno>$drewno_budowa && $metal>$metal_budowa && $kamien>$kamien_budowa && $pobranywiersz['koszary']==0){
      $sql = "UPDATE wioska SET koszary=1 WHERE id=$id_wioski";
