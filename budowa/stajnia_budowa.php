<?php 
 ...
 if($rezultat2 = @$polaczenie->query($sql))
 {
     ...
     $wiersz_uniwersytet = $rezultat2->fetch_assoc();
      ...
      $koszt=(int)$wiersz_stajnia['pieniadze']; 
      $drewno_budowa=(int)$wiersz_stajnia['drewno']; 
      $metal_budowa=(int)$wiersz_stajnia['metal'];
      $kamien_budowa=(int)$wiersz_stajnia['kamien'];  
      if($pieniadze>$koszt && $drewno>$drewno_budowa && $metal>$metal_budowa && $kamien>$kamien_budowa && $pobranywiersz['stajnia']==0){
      $sql = "UPDATE wioska SET stajnia=1 WHERE id=$id_wioski";
